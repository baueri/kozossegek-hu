...
class StaticPages extends Repository
{
    public function getSiteMapUrls(): Collection
    {
        return Pages::query()
            ...
            ->unshift(new SiteMapUrl(loc: route('portal.register_group'), priority: '0.8', changefreq: ChangeFreq::yearly))
            ->unshift(new SiteMapUrl(loc: route('login'), priority: '0.6', changefreq: ChangeFreq::yearly))
            ->unshift(new SiteMapUrl(loc: route('portal.spiritual_movements'), priority: '0.7', changefreq: ChangeFreq::yearly));
    }
}
