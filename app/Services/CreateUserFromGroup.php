...
class CreateUserFromGroup
{
     ...
     * @param \App\Services\App\Repositories\Groups $groups
     */
     ...
     * @return Mailable
     */
    private function getMailable($template, User $user, UserToken $token, Group $group)
    {
        if ($template == 'register_by_group') {
            return RegistrationByGroupEmail::make($user, $token, $group);
        }
