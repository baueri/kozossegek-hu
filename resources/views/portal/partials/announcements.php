@foreach($announcements as $i => $announcement)
    ...
    <div class='announcement'>
        ...
        @endif
        <h3 class='announcement-header text-sm-center my-3 text-center'>
            {{ $announcement->title }}
        </h3>
        <div class='announcement-content'>{{ $announcement->content }}</div>
        <p class="text-center nav-buttons">
            <span class="announcement-no">{{ $i +1 }}</span> / {{ $announcements->count() }} <br/>
            <label class="btn btn-sm btn-common text-primary prev-announcement" for="announcement-{{ $i -1 }}"><u>Előző</u></label>
            <label class="btn btn-sm btn-common text-primary next-announcement" for="announcement-{{ $i +1 }}"><u>Következő</u></label>
        </p>
    </div>
@endforeach
