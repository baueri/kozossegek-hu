...
<head>
    ...
    <title>{{ $subtitle ?? '' }}{{ site_name() }}</title>
    ...
    <mint-yield name="header" />

    <link rel="stylesheet" href="/css/style.css?{{ filemtime(ROOT . 'public/css/style.css') }}">
    <link href="/css/common.css" rel="preload" as="style" onload="this.onload=null;this.rel='stylesheet'" media="all">
</head>

<body class="{{ trim(!is_prod() ? 'demo ' : '') }} {{ $bodyClass ?? '' }}">
